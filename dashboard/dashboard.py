...
day_df = pd.read_csv("dahsboard/day.csv")
day_df.head()
...
def create_proportion_type_rental_df(df):
...
proportion_df = create_proportion_type_rental_df(main_df)
...
# Membuah jumlah penyewaan berdasarkan kondisi cuaca
st.subheader('Type of Rentals')

fig, ax = plt.subplots(figsize=(16, 8))

plt.pie(proportion_df, labels=proportion_df.index, autopct='%1.2f%%', startangle=140)
